...
"""
IOBuffer(str::String) = IOBuffer(convert(Vector{UInt8},str))
IOBuffer(s::SubString{String}) = IOBuffer(view(convert(Vector{UInt8},s.string), s.offset + 1 : s.offset + sizeof(s)))
...
macro b_str(s); :(convert(Vector{UInt8},$(unescape_string(s)))); end
